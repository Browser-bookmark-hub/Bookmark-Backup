    ...
    block.innerHTML = `
        ...
        <div class="config-content" id="realtimeBackupContent" style="display: none; padding: 15px;">
            <div style="font-size: 13px; line-height: 1.6; color: var(--theme-text-secondary); margin-bottom: 15px;">
                ...
                <div id="realtimeBackupDesc2" style="margin-top: 5px;">${lang === 'en' ? 'which may generate many versions in a short time, widen version gaps, and increase storage usage.' : '在很短时间内可能连续生成多个版本，拉大版本跨度并增加存储占用。'}</div>
                <div id="realtimeBackupDesc3" style="margin-top: 5px;">${lang === 'en' ? 'example of count/structure changes: (BKM<span style="color: #4CAF50;">+1</span>, BKM<span style="color: #F44336;">-1</span>, Moved<span style="color: #2196F3;">1</span>, Modified<span style="color: #FF9800;">1</span>).' : '示例（数量/结构变化）：(书签<span style="color: #4CAF50;">+1</span>、书签<span style="color: #F44336;">-1</span>、移动<span style="color: #2196F3;">1</span>、修改<span style="color: #FF9800;">1</span>)。'}</div>
            </div>
        </div>
    `;
